...
struct imp measure_DUT_impedance(int freq){
	...
	impedance.real = (vdut_total.magnitude/vrf_total.magnitude)*cos(vrf_total.phase)*get_resistor(best_resistor) - 100;
	impedance.imag = -(vdut_total.magnitude/vrf_total.magnitude)*sin(vrf_total.phase)*get_resistor(best_resistor);
...
//		vm = make_voltage_measurement(2, 3, SINE_1K);
		struct imp imp1 = measure_DUT_impedance(SINE_1K);
		struct imp imp2 = measure_DUT_impedance(SINE_10K);
		...
		float32_t Reactance[2] = {imp1.imag, imp2.imag};
		struct RLC Ans = calculate_LC(Reactance, IAtAAt, 0);
		Ans.R = (imp1.real+imp2.real)/2;
		//output_number(Ans.R*1000);
		output_RLC(Ans);
		HAL_Delay(100);
    /* USER CODE END WHILE */
