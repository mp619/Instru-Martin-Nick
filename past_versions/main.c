...
//  	  	  	  	  	  	  	-234.05};
  ...
  /* USER CODE BEGIN WHILE */
	while (1) {
		...
		struct imp imp5 = measure_DUT_impedance(SINE_30K);
		...
		float32_t Reactance[3] = {imp3.imag, imp4.imag, imp5.imag};
		struct RLC Ans = calculate_LC(Reactance, IAtAAt, 0);
		//struct RLC Ans = {0};
		//Ans.R = (imp1.real+imp2.real+imp3.real+imp4.real+imp5.real)/5;
		output_number(imp3.imag);
		output_number(imp4.imag);
		output_number(imp5.imag);
		Ans.R = (imp3.real+imp4.real+imp5.real)/3;
		output_RLC(Ans);
		//output_number(phase_Offset*1000);
		HAL_Delay(100);
    /* USER CODE END WHILE */
