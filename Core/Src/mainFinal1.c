...
volatile float32_t phase_Offset = 0;
...
struct voltage_measurement	//Struct for mangnitude and phase of voltage measurment
{
	double magnitude;
	double phase;
};

struct imp	//Struct of impedance real and imag parts
{
	float real;
	float imag;
};

struct RLC	//Struct for RLC values
{
	float R;
	float L;
	float C;
...
void select_measurement_mode(int measurement_mode){	//Set MUX input for InAmp
...
const float32_t A_f32[4] = {
	/* Omega 	1/Omega */
	6283.185307, -0.00015915494,	//1kHz
	62831.85307, -0.000015915494,	//10kHz
...
struct imp measure_DUT_impedance(int freq){
	...
	int best_resistor = get_best_resistor(freq);	//Get best resistor
